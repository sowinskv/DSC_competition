...
class ModelTrainer:
    ...
    def prepare_data(self, data: pd.DataFrame, drop_rows: bool = True, fit_encoder: bool = False) -> tuple[pd.DataFrame, pd.Series | None]:
        if drop_rows:
            # dont drop rows when making final preds
            data = data.dropna()
        else:
        ...
        categorical_columns = [
                "Waluta", "Stan", "Marka_pojazdu", "Model_pojazdu",
                "Generacja_pojazdu", "Rodzaj_paliwa", "Naped", "Skrzynia_biegow",
                "Typ_nadwozia", "Kolor", "Kraj_pochodzenia"
            ]
        ...
        #todo handle this
        data = data.drop(columns=["Wyposazenie", "Data_pierwszej_rejestracji", "Data_publikacji_oferty", "Pierwszy_wlasciciel", "Lokalizacja_oferty", "Wersja_pojazdu"])
    ...
    def train_lightgbm_regressor(self, X_train, y_train, X_test, y_test):
        ...
        lgb.plot_importance(final_model, importance_type='gain', figsize=(10, 10), max_num_features=30)
        plt.savefig(f"{self.results_path}\\{output_folder_name}\\feature_importance_{output_folder_name}_gain.png")
        lgb.plot_importance(final_model, importance_type='split', figsize=(10, 10), max_num_features=30)
