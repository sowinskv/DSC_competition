...
class ModelTrainer:
    ...
    def prepare_data(
        ...
        categorical_low_cardinality = [col for col in categorical_columns if data[col].nunique() < 5]
        categorical_high_cardinality = [col for col in categorical_columns if data[col].nunique() >= 5]
        ...
        # target encoding
        if fit_encoder:
            self.encoder = ce.TargetEncoder(cols=categorical_high_cardinality)
            data[categorical_high_cardinality] = self.encoder.fit_transform(data[categorical_high_cardinality], y)
        else:
            data[categorical_high_cardinality] = self.encoder.transform(data[categorical_high_cardinality])

        #one hot encoding
        data = pd.get_dummies(data, columns=categorical_low_cardinality, drop_first=True)
    ...
    def train_lightgbm_regressor(self, X_train, y_train, X_test, y_test):
        ...
        def objective(trial):
            params = {
                ...
                'min_split_gain': trial.suggest_float('min_split_gain', 0.0, 0.5),  # Minimum loss reduction to split
                'min_child_weight': trial.suggest_float('min_child_weight', 0.001, 10.0),
                'feature_fraction': trial.suggest_float('feature_fraction', 0.5, 1.0),  # Randomly select features
                'feature_fraction_bynode': trial.suggest_float('feature_fraction_bynode', 0.5, 1.0),
                'bagging_fraction': trial.suggest_float('bagging_fraction', 0.5, 1.0),  # Subsample rows
                'bagging_freq': trial.suggest_int('bagging_freq', 1, 10),  # Bagging frequency (every k rounds)
                'categorical_feature': 'auto',
                # # Histogram Optimization (speed vs accuracy tradeoff)
                # 'max_bin': trial.suggest_int('max_bin', 100, 300),  # Number of bins for feature discretization
                # 'min_data_in_bin': trial.suggest_int('min_data_in_bin', 3, 50),  # Minimum samples per bin
            }
